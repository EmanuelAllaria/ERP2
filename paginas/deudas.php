                            ...
                            <tbody id="lista_facturas">
                                ...
                                while ($row = mysqli_fetch_assoc($con_facturas)) {
                                    ...
                                    $consulta_pagos = $link->query("SELECT
                                                                        pagos_agrupados.id,
                                                                        pagos_agrupados.id_proveedor,
                                                                        pagos_agrupados.tipo_pago,
                                                                        pagos_agrupados.fecha,
                                                                        pagos_agrupados.fecha_emision,
                                                                        pagos_agrupados.observaciones,
                                                                        SUM(
                                                                            CASE 
                                                                                WHEN pagos_agrupados.cheque_rechazado = 0 OR pagos_agrupados.cheque_rechazado IS NULL THEN pagos_agrupados.monto
                                                                                ELSE 0
                                                                            END
                                                                        ) AS monto
                                                                    FROM (
                                                                        SELECT 
                                                                            ...
                                                                            fp.tipo_pago,
                                                                            fp.observaciones,
                                                                            COALESCE(MAX(fc.cheque_rechazado), 0) AS cheque_rechazado
                                                                        FROM
                                                                            facturas_pagos fp
                                                                        LEFT JOIN facturas_cheques fc ON fp.id = fc.id_pago
                                                                        WHERE
                                                                            fp.id_proveedor = '{$row['id_proveedor']}'
                                                                        GROUP BY 
                                                                            fp.id, fp.id_proveedor, fp.monto, fp.fecha_emision
                                                                    ) AS pagos_agrupados
                                                                    GROUP BY
                                                                        pagos_agrupados.id_proveedor
                                                                    ORDER BY
                                                                        MIN(pagos_agrupados.fecha_emision) ASC");
                                    ...
                                    while ($pago = mysqli_fetch_assoc($consulta_pagos)) {
                                        ...
                                        echo "<td style='width: 14.2857142857142%;word-break: break-all;'><a href='index.php?pagina=pagos_factura&proveedor={$row['id_proveedor']}' class='btn btn-info btn-lg' role='button'>Ver Pagos</a></td>";
                                        echo "</tr>";
                                    }
                                ...
                                if ((!isset($busqueda) || $busqueda === '') && isset($proveedores_faltantes[0])) {
                                    ...
                                    while ($pago_a_favor = mysqli_fetch_assoc($consulta_pagos_a_favor)) {
                                        ...
                                        echo "<td style='width: 14.2857142857142%;word-break: break-all;color:green;'><a href='index.php?pagina=pagos_factura&proveedor={$pago_a_favor['id_proveedor']}' class='btn btn-info btn-lg' role='button'>Ver Pagos</a></td>";
                                        echo "</tr>";
                                    }
                                }
                                ?>
