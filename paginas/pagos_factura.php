<?php if (isset($_GET['add']) && $_GET['add'] == '1') {
  include('clientes_add.php');
    ...
    <div class="row page-titles">
      <div class="col-md-12">
        <h4 class="text-white">Listado de Pagos a Proveedores</h4>
      </div>
      <div class="col-md-6">
        <ol class="breadcrumb">
          ...
          <div class="card-body">
            ...
            <div class="row">
              ...
              <div class="col-md-2"><small class="form-control-feedback">Forma pago </small><br>
                <select class="form-control" id="formasel">
                  ...
                  while ($row = mysqli_fetch_array($busca_f)) {
                    echo '<option value="' . (strtolower($row['detalle_formapago'])) . '"';
              ...
              <div class="col-md-2" style="margin-top:17px">
                ...
                <?php if (isset($_GET['d']) || isset($_GET['h'])) { ?><a href="index.php?pagina=pagos_factura">Quitar Filtros</a><?php } ?>
              </div>
              ...
              <div class="col-md-2" style="align-self: center;">
                <div id="totales_formas">
                  ...
                  if (isset($_GET['f']) && $_GET['f'] != '') {
                    if ($_GET['f'] === 'mercado pago') {
                      $busqueda = $busqueda . " and facturas_pagos.tipo_pago = 'mp'";
                    } else {
              ...
              <table id="clientes_lista" class="table m-t-30 table-hover contact-list footable-loaded footable" data-page-size="10">
                ...
                <tbody>
                  ...
                  while ($row = mysqli_fetch_assoc($con_facturas)) {
                    $id_pago = $row['id'];
                    $monto_cheque = $link->query("SELECT SUM(monto) as total
                                                FROM facturas_cheques
                                                WHERE id_pago = '$id_pago'
                                                ORDER BY id ASC");
                    $monto += $monto_cheque->fetch_assoc()['total'];
                    ...
                    echo "<td style='" . ($row['pago_favor'] == 1 ? 'color:green;' : '') . "'>" . ($row['tipo_pago'] === 'mp' ? $row['nro_comprobante'] : '') . "</td>";
                    ...
                    echo "<td style='" . ($row['pago_favor'] == 1 ? 'color:green;' : '') . "'>$" . number_format(intval($row['monto']), 2, ',', '.') . "</td>";
                    ...
                    if ($row['tipo_pago'] === 'cheque') {
                      $con_cheque = $link->query("SELECT facturas_cheques.*, clientes.nombre_clientes, clientes.apellido_clientes, CASE WHEN facturas_cheques.cheque_rechazado = 1 THEN - ABS(facturas_cheques.monto) ELSE ABS(facturas_cheques.monto) END AS monto
                                                  FROM facturas_cheques
                                                  LEFT JOIN clientes ON clientes.id_clientes = facturas_cheques.origen
                                                  WHERE facturas_cheques.id > 0 AND facturas_cheques.id_pago = '$id_pago'
                                                  ORDER BY facturas_cheques.id ASC;");

                      while ($row_cheque = mysqli_fetch_assoc($con_cheque)) {
                        $numero_cheque = $row_cheque['numero_cheque'];
                        $result = $link->query("SELECT * FROM imagen_cheque WHERE numero_cheque = $numero_cheque");
                        if ($result) {
                          $imagen_cheque = $result->fetch_assoc();
                          $imagen_cheque = $imagen_cheque ? $imagen_cheque['url_imagen'] : null;
                        }
                        ...
                        echo "<td>{$row_cheque['nombre_clientes']} {$row_cheque['apellido_clientes']}</td>";
                        echo "<td>" .
                          ...
                          " " . ($imagen_cheque ? "<a href='uploads/cheques/" . $imagen_cheque . "' target='_blank'><i class='fa-solid fa-image'></i></a>" : "") . "</td>";
  ...
  <script>
    ...
    function filtrar_vende() {
      ...
      if (datodesde) { // require a URL
        window.location = 'index.php?pagina=pagos_factura&d=' + datodesde + '&h=' + datohasta + '&p=' + datovendedor + '&f=' + datoforma; // redirect
      }
      return false;
    };
  </script>
