                        ...
                        <table id="facturas_lista" class="table m-t-30 table-hover contact-list footable-loaded footable" data-page-size="10">
                            ...
                            <tbody id="lista_facturas">
                                ...
                                while ($row = mysqli_fetch_assoc($con_facturas)) {
                                    $consulta_pagos = $link->query("SELECT * FROM facturas_pagos WHERE id_proveedor='{$row['id_proveedor']}' ORDER BY fecha_emision ASC");
                                    ...
                                    echo "<td style='color: red;'>$" . number_format(($row['total_factura'] > 0) ? $row['total_factura'] : 0, 2, ',', '.') . "</td>";
                                    echo "<td style='color: green;'>$" . number_format(($row['total_factura'] < 0) ? abs($row['total_factura']) : 0, 2, ',', '.') . "</td>";
                                    echo "<td>$" . number_format($saldo_factura, 2, ',', '.') . "</td>";
                                    ...
                                    while ($pago = mysqli_fetch_assoc($consulta_pagos)) {
                                        ...
                                        echo "<tr>";
                                        echo "<td></td>";
                                        echo "<td>{$pago['fecha_emision']}</td>";
                                        echo "<td></td>";
                                        echo "<td style='color: green;'>$" . number_format($pago['monto'], 2, ',', '.') . "</td>";
                                        echo "<td>$" . number_format($saldo_factura, 2, ',', '.') . "</td>";
                                ...
                                <tr>
                                    <td colspan="12">
                                        <div class="text-right">
                                            <ul class="pagination pagination-split m-t-30 m-b-30"></ul>
                                        </div>
                                    </td>
                                </tr>
                            </tfoot>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const totalPeriodoElement = document.getElementById('total_periodo');
        const saldoFinal = parseFloat("<?php echo $saldo_final; ?>");
        totalPeriodoElement.innerHTML = `Total del periodo: $${saldoFinal.toFixed(2)}`;
    });
</script>