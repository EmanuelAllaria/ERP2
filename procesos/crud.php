...
if ($_SESSION['usuario'] != '') {
  ...
  if (isset($_POST['accion']) && $_POST['accion'] == 'add_compras') {
    ...
    $iibb = $_POST['iibb'];
    $por_iibb = $_POST['por_iibb'];

    $per_iva = $_POST['per_iva'];
    $por_per_iva = $_POST['por_per_iva'];

    $imp_int = $_POST['imp_int'];
    $por_imp_int = $_POST['por_imp_int'];

    $tasa = $_POST['tasa'];
    $por_tasa = $_POST['por_tasa'];
    ...
    $inserta = $link->query("INSERT INTO compra_mercaderia SET
      ...
      ingresacuenta_corriente = '$c_corriente',
      vencimiento_compram = '$vencimiento',
      estado_compram = '$estado'
    ");

    if ($c_corriente == 1) {
      $inserta_gasto = $link->query("INSERT INTO facturas SET
        nro_factura = '$nro_comp',
        id_proveedor = '$prov',
        fecha = '$fecha_compra',
        tipo = '$tipo_comp',
        monto = '$total',
    ...
    $id_compram = $link->query("SELECT id_compram FROM compra_mercaderia ORDER BY id_compram DESC LIMIT 1");
    $rowmer = mysqli_fetch_array($id_compram);
    $id_ult_compram = $rowmer['id_compram'];

    $inserta2 = $link->query("INSERT INTO impuesto_compra_mercaderia SET
      por_iva = '$por_iva',
      iva = '$iva',
      por_iibb = '$por_iibb',
      iibb = '$iibb',
      por_per_iva = '$por_per_iva',
      per_iva = '$per_iva',
      por_imp_interno = '$por_imp_int',
      imp_interno = '$imp_int',
      tasa = '$tasa',
      por_tasa = '$por_tasa',
      id_compra_mercaderia = '$id_ult_compram',
      sociedad = '$sociedad'
    ");
    ...
    for ($i = 0; $i < count($itemDecodificados); $i++) {
      ...
      $precio = $itemDecodificados[$i]['precio'];
      $consulto_datos = $link->query("SELECT stock_producto,presentacion_producto from productos where id_producto ='$id_producto'");
      $row = mysqli_fetch_array($consulto_datos);

      $stockActual = intval($row[0]);
      $presentacion_producto = intval(($row[1]));
      $cantidad_final = $cantidad  /  $presentacion_producto;

      $stockFinal = $stockActual + $cantidad_final;
      $precio_caja = $precio * $presentacion_producto;

      $por_iva = floatval($por_iva);
      $por_iibb = floatval($por_iibb);
      $por_per_iva = floatval($por_per_iva);
      $por_imp_int = floatval($por_imp_int);
      $por_tasa = floatval($por_tasa);

      $precio_iva = $precio * ($por_iva / 100);
      $precio_iibb = $precio * ($por_iibb / 100);
      $precio_per_iva = $precio * ($por_per_iva / 100);
      $precio_imp_int = $precio * ($por_imp_int / 100);
      $precio_tasa = $precio * ($por_tasa / 100);

      $precio_impuestos = $precio + $precio_iva + $precio_iibb + $precio_per_iva + $precio_imp_int + $precio_tasa;

      $precio_final = $precio_impuestos * $presentacion_producto;

      $inserta = $link->query("UPDATE productos SET stock_producto='$stockFinal', costo_producto = '$precio_final' where id_producto='$id_producto' ");
    }


    if ($inserta && $inserta_gasto && $inserta2) {
      ...
      for ($i = 0; $i < count($itemDecodificados); $i++) {
        ...
        $cantidad = intval($itemDecodificados[$i]['cant']);
        $precio = $itemDecodificados[$i]['precio'];
        $importe = $itemDecodificados[$i]['importe'];
        $inserta_compra = $link->query("INSERT INTO productos_comprados SET idCMercaderia = '$id_ult_compra', idProducto = '$id_producto', cantidad = '$cantidad',precio ='$precio', importe ='$importe'");
  ...
  if (isset($_POST['accion']) && $_POST['accion'] == 'elimina_factura') {
    $id = $_POST['id'];

    $delete = $link->query("DELETE FROM facturas WHERE id = '$id'");
  ...
  if (isset($_POST['accion']) && $_POST['accion'] == 'check_factura') {

    $nro_factura = $_POST['nro_factura'];

    $nro_factura_escapado = $link->real_escape_string($nro_factura);


    $consulta_factura = $link->query("SELECT COUNT(*) as count FROM facturas WHERE nro_factura = '$nro_factura_escapado'");

    if ($consulta_factura) {
      $resultado = $consulta_factura->fetch_assoc();
  ...
  if (isset($_POST['accion']) && $_POST['accion'] == 'check_comprobante') {

    $numero_comprobante = $_POST['numero_comprobante'];


    $numero_comprobante_escapado = $link->real_escape_string($numero_comprobante);


    $consulta_comprobante = $link->query("SELECT COUNT(*) as count FROM compra_mercaderia WHERE numcom_compram = '$numero_comprobante_escapado'");

    if ($consulta_comprobante) {
      $resultado = $consulta_comprobante->fetch_assoc();
      if ($resultado['count'] > 0) {
        echo 'FALSE';
      } else {
        echo 'TRUE';
  ...
  if (isset($_POST['accion']) && $_POST['accion'] == 'get_facturas') {
    ...
    // Suma total de los montos de las facturas
    $consulta_facturas = $link->query("SELECT SUM(monto) AS total_facturas 
                                       FROM facturas f 
    ...
    $result_facturas = mysqli_fetch_assoc($consulta_facturas);
    $total_facturas = $result_facturas['total_facturas'];

    // Suma total de los montos de los pagos
    $consulta_pagos = $link->query("SELECT SUM(monto) AS total_pagos 
                                    FROM facturas_pagos 
                                    WHERE id_proveedor='$proveedor'");
    $result_pagos = mysqli_fetch_assoc($consulta_pagos);
    $total_pagos = $result_pagos['total_pagos'];

    // Calcular la diferencia entre facturas y pagos
    $saldo_final = $total_facturas - $total_pagos;

    if ($saldo_final) {
      echo json_encode($saldo_final);
    } else {
      echo 'FALSE';
    }
  }
  ...
  if (isset($_POST['accion']) && $_POST['accion'] == 'add_facturas_pago') {
    ...
    $nro_comprobante = $_POST['nroComprobante'];
    $cuit = $_POST['cuit'];
    $monto = $_POST['monto'];
    $origen = $_POST['origen'];
    ...
    if ($_POST['tipo_pago'] !== 'cheque') {
      $inserta = $link->query("INSERT INTO facturas_pagos SET id_proveedor='$proveedor', tipo_pago='$tipo_pago', fecha='$fecha', banco='$banco', fecha_emision='$fecha_emision', fecha_cobro='$fecha_cobro', titular='$titular', nro_comprobante='$nro_comprobante', cuit='$cuit', monto='$monto', origen='$origen'");
      $id = mysqli_insert_id($link);
    } else {
      $inserta = $link->query("INSERT INTO facturas_pagos SET id_proveedor='$proveedor', tipo_pago='$tipo_pago', fecha='$fecha', monto='$monto', titular='$titular'");
      $id = mysqli_insert_id($link);
